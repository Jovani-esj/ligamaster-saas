-- Script corregido para arreglar RLS y acceso a ligas
-- Ejecutar en el editor SQL de Supabase

-- 1. Primero, verificar estructura actual de la tabla
...
-- 3. Crear nuevas políticas RLS corregidas

-- Política para que CUALQUIER USUARIO pueda ver todas las ligas (sin autenticación requerida)
CREATE POLICY "Enable anonymous and authenticated read access to ligas" ON ligas
...
-- 5. Verificar las políticas nuevas
SELECT 
  schemaname,
  tablename,
  policyname,
...
-- 6. Verificar datos de ligas después de actualización
SELECT 
  id,
  nombre_liga,
  slug,
...
-- 7. Crear vista pública alternativa (si prefieres evitar RLS completamente)
...
-- 8. Dar permisos públicos a la vista
...
WHERE table_name = 'public_all_ligas';
